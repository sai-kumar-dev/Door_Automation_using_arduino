...
bool prevEmergencyStopped = false;
...
void loop() {
  ...
  if (digitalRead(EMERGENCY_STOP) == LOW) {
    ...
    if (isEmergencyStopped) {
      ...
      Serial.println("✅ Emergency Stop Deactivated. System Ready.");
    }
  }

  if (isEmergencyStopped) {
    return; // Prevents any further actions while stopped
  }
  ...
  if (!reached) {
    if (digitalRead(HOME) == HIGH) {
      ...
      Serial.println("✅ Homing Complete! System Ready. Auto-close Timer Started.");
    }
  }
